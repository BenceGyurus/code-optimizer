...
Ennél a feladatnál azt próbáltam ki, hogy lehet-e LLM-eket nem csak sima kódgenerálásra használni, hanem egy mérhető, ellenőrzött optimalizálási folyamat részeként. Nem azt szerettem volna, hogy a modell ránézésre átírjon valamit, aztán elhiggyük neki, hogy gyorsabb lett. Inkább az volt a cél, hogy előbb mérjen, nézze meg a kódot, válasszon egy konkrét célpontot, írjon hozzá patch-et, majd a program tesztekkel és újraméréssel döntse el, hogy ez tényleg elfogadható-e.

A programot a saját instrukcióim és döntéseim alapján készítettem, de a megvalósítás nagy részében Codexet használtam fejlesztőtársként. Ez nem úgy nézett ki, hogy egyszer beírtam egy promptot és kész lett minden, hanem sok kisebb körből állt. Megmondtam, milyen működést szeretnék, futtattam a programot, megnéztük az eredményeket, aztán azok alapján javítottunk rajta. A Codex főleg a kódolásban, refaktorálásban, promptok átírásában, shell scriptek elkészítésében és a dokumentáció összerakásában segített.

A végére két fő futtatási mód lett belőle. Az `optimizer run` egyetlen optimalizálási sessiont indít, az `optimizer evaluate` pedig több modellből, prompt packből és ismétlésből rak össze egy mérési mátrixot. Nekem főleg az utóbbi volt érdekes, mert így nem csak egy-egy sikeres futást lehetett nézni, hanem azt is, hogy a különböző modellek és promptolási stratégiák mennyire stabilak. A futások végén külön mappába kerülnek a sessionök, az összesített `CSV` és `YAML` eredmények, a Markdown riport és a diagramok. A runtime és a hardveres mérések sem egyetlen futásból jönnek, hanem ismétlések átlagából, a végső méréseknél például 15 futásból.
...
Az egész program arra épül, hogy az AI ne közvetlenül, kontroll nélkül írja át a kódot. Inkább egy döntéshozó szerepet kap egy olyan folyamatban, ahol előbb mérés van, és csak utána módosítás. A modell kap baseline futási időt, teszteredményt, profilozási adatokat, hardveres metrikákat és kódrészleteket. Ezekből kell kitalálnia, hogy hol érdemes hozzányúlni. Nem az a cél, hogy minél több fájlt átírjon, hanem hogy találjon egy konkrét szűk keresztmetszetet, és arra adjon egy lehetőleg kicsi, ellenőrizhető javítást.
...
A modell nem kapja meg a teljes nyers logot, mert az gyorsan túl sok token lenne, főleg kisebb modelleknél. Ehelyett rövidített, strukturált adatokat adok neki: tesztösszefoglalót, benchmark átlagot/minimumot/maximumot, profilozási hotspotokat, hardveres metrikákat, aktuális állapotot, engedélyezett toolokat és célzott kódrészleteket. Többfájlos projektnél ehhez jön még fájllista, függvényvázlat és szűkebb kódkontextus is. Így a modell nem egy nagy, rendezetlen szöveget kap, hanem egy mérési csomagot, amiből dönteni tud.

A kimenetből nem csak az derül ki, hogy sikerült-e egy futás. Látszik a patch, a tesztek eredménye, a baseline és optimalizált futási idő, illetve a speedup is. Az `evaluate` futás ezekből készít összesített `CSV` és `YAML` fájlokat, `report.md`-t és diagramokat. Ezekből már össze lehet hasonlítani a modelleket, prompt packeket, toolhasználatot, LLM hívásszámot és a hardveres metrikák változását is.
...
Ha a patch átment a teszteken, a program újraméri az optimalizált kódot. Ugyanazok a benchmark és hardveres mérési parancsok futnak, ugyanannyi ismétléssel, mint baseline-nál. Az `evaluate_result` ezután összeveti a két állapotot. Ha a gyorsulás elég jó, a session `DONE` lesz. Ha nincs javulás, vagy elfogyott a budget, akkor a futás megáll, és sikertelen vagy nem elég jó optimalizációként kerül a riportba.
...
Nem egyetlen nagy promptot használtam, hanem prompt packeket. Ennek az volt az oka, hogy a program nem egy darab választ vár a modelltől, hanem egy folyamatot vezérel. Más szöveg kell akkor, amikor csak a következő toolt kell kiválasztani, más akkor, amikor célpontot elemez, más akkor, amikor patch-et ír, és más akkor is, amikor az eredményt értékeli. Ha ez egyetlen nagy promptban lenne, sokkal könnyebben összemosódnának a szerepek, és egy hibás részt is nehezebb lenne külön javítani. Így viszont ugyanaz a rendszerlogika marad, de külön lehet finomítani a döntési, elemzési, patch-generálási és értékelési promptot.
...
Két fő tesztprogrammal dolgoztam. Az első a [heavy_compute.py](examples/heavy_compute.py), ami egy egyfájlos, direkt számításigényes Python program saját unittest tesztekkel. Ezen jól látszott, hogy a modellek megtalálják-e az egyértelmű algoritmikus hibát, és tudnak-e úgy gyorsítani rajta, hogy a tesztek továbbra is átmenjenek. A második a [complex_pipeline](examples/complex_pipeline), ami már több fájlból áll. Ehhez külön tesztfájl is tartozik: [test_market_sim.py](examples/complex_pipeline/test_market_sim.py). Ezt azért tettem mellé, mert a valódi projektekben általában nem egyetlen fájlon belül kell gondolkodni.

A Debianos futtatásokhoz külön shell scriptek készültek. Ezek főleg arra jók, hogy ne kézzel kelljen mindig újra megadni a modelleket, prompt packeket, `perf` countereket, ismétlésszámokat és benchmark parancsokat.
...
A teszteket Debian 12 alatt futtattam, mert a hardveres mérésekhez Linuxos `perf` countereket használtam. Az OpenRouteres modellek külső szolgáltatáson keresztül mentek, a self-hostolt Qwen 2.5 Coder 7B viszont helyben futott, egy NVIDIA P104-100 videókártyán, 8 GB VRAM-mal. Ezt azért írom le külön, mert egy lokális modellnél nem csak a modell számít, hanem az is, milyen vas szolgálja ki.

A végső teszteknél kétféle kód szerepelt. A `heavy_compute.py` egyfájlos, erősen számításigényes példa volt. Itt a legerősebb modellek szinte mindig a `segmented_prefix_sums_slow` részt találták meg, és az O(n²) jellegű számítást O(n) futó összeges megoldásra cserélték. Emiatt több modellnél is 7-8x körüli gyorsulás jött ki. Ez elsőre gyanúsan egységesnek tűnhet, de ebben az esetben inkább az látszik belőle, hogy volt a kódban egy nagyon domináns, könnyen mérhető hotspot.
...
A hardveres méréseknél érdekes volt, hogy a cache hit-rate önmagában nem elég jó sikerességi mutató. Volt olyan futás, ahol a cache arány kicsit romlott, de a program sokkal gyorsabb lett, mert egyszerűen kevesebb munkát végzett. Emiatt nálam a runtime maradt az elsődleges metrika, a cache, L1, branch és LLC adatok pedig inkább magyarázó adatok lettek. Az LLC counterekkel az is kiderült, hogy Linuxon sem garantált minden számláló elérhetősége, ez függ a géptől, kerneltől és virtualizációtól is.

A többfájlos példánál az jött ki, hogy a profilozási eredményt nem szabad vakon olvasni. A cProfile kumulatív ideje néha olyan függvényre mutat, ami csak meghív sok másik drága részt, de nem ott kell optimalizálni. Ilyenkor jobb kontextus kell: fájlok közötti kapcsolat, self-time, hívási lánc és célzott kódrészlet. Emiatt került bele később az is, hogy nagyobb projektnél ne egyben kapja meg a modell az egész kódot, hanem a program logika alapján tördelje és célzottabban adja oda.

Összességében nekem az lett a feladat fő tanulsága, hogy az AI nem önmagában egy varázslatos optimalizáló, hanem egy mérnöki folyamat egyik eleme. A jó eredményt nem csak a modell adja, hanem az, hogy van körülötte mérés, guardrail, teszt, rollback, budget és riportolás. Így már nem csak az látszik, hogy az AI átírt valamit, hanem az is, hogy mit változtatott, miért, hányszor próbálkozott, sikeres volt-e, és mennyit gyorsult tőle a program.
...
2026.03.26 körül a GeminiCLI-val is volt egy érdekes köröm. Korábban többször előfordult, hogy 20-30 percig nem válaszolt, ezért félbeszakítottam. Később viszont meglepően jól működött olyan feladatoknál, ahol egy konkrét feature-t kellett megvalósítania, teszteket írnia hozzá, majd addig javítania, amíg működött. Néha viszont így is véletlenszerűen leállt munka közben. Emiatt végül nem egyetlen eszközre építettem a projektet, hanem több modellt és több futtatási módot próbáltam ki.
